...
BlockEvents.broken('meldworks:living_farmland', event => {
    ...
    if (data && data.absorbed_sum > 0) {
        // Drop indicator dust based on absorbed sum
...
BlockEvents.rightClicked(event => {
    ...
    if (item.id == 'meldworks:mechanical_meter' || item.id == 'meldworks:agricultural_analyzer') {
        if (player.shiftKeyDown) {
            ...
            // Agricultural Analyzer: Exact values
            ...
            if (data.is_resting) player.tell(Text.red("Soil is currently resting."))
        }
        return
    }
    ...
    if (item.id == 'meldworks:indicator_dust') {
        data.reaction_expiry = level.time + 1200 // 60 seconds
        if (!player.creativeMode) item.count--
    ...
    if (item.hasTag('meldworks:fertilizers')) {
        ...
        let amount = 0
        if (tier == 'poor') amount = 15
        if (tier == 'good') amount = 35
        if (tier == 'extravagant') amount = 65
...
// Use Forge events to catch sub-season changes for the lazy reset
// Note: In KubeJS 6, we use ForgeEvents.onEvent
